...
poly(p, k, l) = ( 
	(factorial(big(k+1)) / (factorial(big(l)) * factorial(big(k - l + 1)) ) ) * 
	(p^l) * 
	(1-p)^(k + 1 - l)
)
...
r_k(p, k; B=5, b=1, r=0.5) = [poly(p, k, l)*u_S(k, l, B=B, b=b, r=r) for l in 0:(k+1)]
...
w_S(x, n, p; B=5, b=1, C=0.1, r=0.5, f=1.5) = sum([
	poly(x, n-2, k)*v_S(p, k, B=B, b=b, C=C, r=r, f=f)
	for k in 0:1:(n-1)
])